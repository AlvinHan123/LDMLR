...
# from NormLayer import NormedLinear
...
class ResNet(nn.Module):
    ...
    def forward(self, x, crt=False):   # take cifar10 as an example: x.shape(64,3,32,32)
        out = F.relu(self.bn1(self.conv1(x)))  # shape: (64,16,32,32)
        out = self.layer1(out)  # shape: (64,16,32,32)
        out = self.layer2(out)  # shape: (64,32,16,16)
        out = self.layer3(out)  # shape: (64,64,8,8)
        out = F.avg_pool2d(out, out.size()[3])  # shape: (64,64,1,1)
        out = out.view(out.size(0), -1)  # shape: (64,64)

        if not crt:
            out = self.fc(out)   #   # shape: (64,10)
        return out

    def forward_no_fc(self, x, crt=False):   # take cifar10 as an example: x.shape(64,3,32,32)
        out = F.relu(self.bn1(self.conv1(x)))  # shape: (64,16,32,32)
        out = self.layer1(out)  # shape: (64,16,32,32)
        out = self.layer2(out)  # shape: (64,32,16,16)
        out = self.layer3(out)  # shape: (64,64,8,8)
